...
export function buildMockReview(orderFile: string, mockDataFile: string, batchId = "batch-preview"): MockReviewResult {
  ...
  const dtos: ReviewLineDto[] = reviewLines.map((line, index) => {
    ...
    return {
      ...
      suggestedWarehouseNo: line.suggestedMainQty > 0 ? "001" : line.suggestedNearExpiryQty > 0 ? "LINQI" : "",
      suggestedWarehouseName: line.suggestedMainQty > 0 ? "主仓" : line.suggestedNearExpiryQty > 0 ? "临期仓" : "",
      status: line.status,
      decision,
      approvedShipQty: decision === "ship" ? line.suggestedShipQty : 0,
      fulfillmentWarehouseNo: decision === "ship" ? (line.suggestedMainQty > 0 ? "001" : "LINQI") : "",
      fulfillmentWarehouseName: decision === "ship" ? (line.suggestedMainQty > 0 ? "主仓" : "临期仓") : "",
      reason: "",
      priority: false,
      priorityReason: "",
    };
