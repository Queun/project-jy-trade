...
export function buildReviewLines(
  ...
  inventoryByBarcode: Map<string, InventorySnapshot>,
): ReviewLine[] {
  ...
  return orderLines.map((line): ReviewLine => {
    ...
    const shortageAfterMain = line.orderQty - suggestedMainQty;
    const suggestedNearExpiryQty = Math.min(shortageAfterMain, nearBefore);
