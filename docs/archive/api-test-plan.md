...
## 2026-07-03 测试 API 与正式只读 API 边界

当前确认：旺店通测试 API 是虚拟测试环境，不能作为甲方真实业务数据来源。测试 API 返回的商品、库存、仓库和订单状态只适合做接口连通性、签名、参数和字段结构验证。

项目规则：
- 真实样例订单验证、商品档案同步、库存查询、真实初审，必须使用正式 API profile。
- 正式 API 只允许调用已确认的只读接口；当前核心只读接口是 `goods.Goods.queryWithSpec` 和 `wms.StockSpec.search2`。
- 测试 API 不再用于判断商品是否存在、库存是否充足、仓库分类是否正确。
- 正式只读验证失败时必须暂停流程并提示用户，不允许自动切回测试 API 或 mock 数据继续生成正式审核依据。
- 任何写入类接口，即名称或能力涉及 `push`、`upload`、`create`、`update`、`delete`、导入、推单、创建订单、修改库存等，都必须单独制定测试计划并经人工确认后才可触碰。
...
该设置仅表示读取正式店铺真实数据，不改变当前“不写入旺店通”的安全边界。
