...
审核商品搜索以外部货品码优先、再按外部条码、做单码、规格码和商品名形成商品分组。只有搜索结果唯一对应一个商品时，前端才允许发起批量不发货；API 校验全部明细属于当前批次，统一把最终数量归零、清空仓库、保留备注并记录批次审计。
...
- 后台组合任务先增量同步普通商品，再增量同步组合装及组件关系，然后按 40 个 SKU 一批、至少 1.1 秒请求间隔同步库存；频控或并发错误按 `WDT_STOCK_SYNC_RETRY_DELAYS_MS` 重试。两类商品档案和所有库存批次完整成功后，才把任务状态原子更新为 `success`。
...
- 普通商品、组合装的完整同步和快速同步统一按 `Asia/Shanghai` 序列化旺店通 `start_time` / `end_time`；日期形式的手工边界按上海自然日解释，避免无时区参数被错当成 UTC 而延迟发现当天变化。
