...
1. 后台按 `Asia/Shanghai` 每小时整点同步商品档案和全量库存快照；操作前也可在设置中手动立即同步。
2. 页面上传订货单并创建正式批次。
...
- `allowStaleCache=true` 只用于临时排查商品档案缓存；库存快照即使超过一小时仍可使用并明确提醒，不阻断导入和审核。
- 自动测试默认关闭整点同步，不能调用真实旺店通；需要显式验证调度时注入 mock client 和测试时钟。
- 开发环境热更新时可用 `WDT_AUTO_SYNC_ENABLED=false npm run dev:api`，避免在本地无成功快照或快照超过一小时的情况下，API 重载触发启动补偿同步。
