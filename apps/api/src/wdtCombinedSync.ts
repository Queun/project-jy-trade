...
export interface CombinedSyncRepository {
  ...
  writeStockBatch(runId: string, requestedSpecNos: string[], rows: WdtStockRow[], syncedAt: string): Promise<number>;
  activateRun(runId: string, finishedAt: string): Promise<void>;
...
export async function executeCombinedSync(repository: CombinedSyncRepository, stockClient: StockLookupClient, runId: string) {
  try {
    ...
    for (const [index, batch] of batches.entries()) {
      ...
      try {
        response = await queryStockWithRetry(stockClient, batch);
      } catch (error) {
      ...
      rowCount += await repository.writeStockBatch(runId, batch, response.data?.detail_list ?? [], now);
    ...
    await repository.activateRun(runId, now);
  } catch (error) {
...
async function queryStockWithRetry(stockClient: StockLookupClient, specNos: string[]) {
  ...
  for (const delayMs of [0, ...retryDelays]) {
    ...
    try {
      const response = stockClient.queryStocks
        ? await stockClient.queryStocks(specNos)
        : await stockClient.queryStock(specNos[0] ?? "");
